...
export function BusinessCardPreview({
  ...
  const visibleActions = isPremium ? displayActions : displayActions.slice(0, 3);
  const hints = [...labels.hints];
  if (hours) hints[2] = hours;
  if (reviews) hints[3] = reviews;
  hints[4] = gallery ?? `${labels.hints[4]} · ${isPremium ? '20' : '≤ 5'}`;
  ...
  ].filter(({ label }) => (isPremium ? networks.includes(label) : label === 'Facebook'));
  ...
  const sectionOrder = isPremium ? [2, 3, 4, 1] : [0, 1, 2, 4];
