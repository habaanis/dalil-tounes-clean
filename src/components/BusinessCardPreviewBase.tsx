...
const COPY: Record<BusinessCardPreviewLanguage, PreviewCopy> = {
  fr: {
    productPremium: 'CV BUSINESS PREMIUM',
    productArtisan: 'CV BUSINESS ARTISAN',
    certified: '★ CERTIFIÉ DALIL TOUNES',
    introEyebrow: 'PRÉSENTATION',
    introTitle: 'Un professionnel proche de vos projets',
    introText: 'Une présentation claire de votre activité, de votre savoir-faire et de ce qui vous différencie.',
    read: 'Lire la présentation',
    services: 'Services proposés',
    photos: 'Photos et réalisations',
    hours: 'Horaires',
  ...
  ar: {
    ...
    introEyebrow: 'نبذة عن المؤسسة',
    introTitle: 'مهني قريب من مشاريعكم',
    introText: 'عرض واضح للنشاط والخبرة وما يميز هذا المهني.',
    read: 'قراءة التعريف',
    services: 'الخدمات المقترحة',
    photos: 'الصور والإنجازات',
    hours: 'أوقات العمل',
  ...
  en: {
    productPremium: 'PREMIUM BUSINESS CV',
    productArtisan: 'ARTISAN BUSINESS CV',
    certified: '★ DALIL TOUNES CERTIFIED',
    introEyebrow: 'ABOUT',
    introTitle: 'A local professional for your projects',
    introText: 'A clear presentation of the activity, expertise and what makes this professional different.',
    read: 'Read the presentation',
    services: 'Services',
    photos: 'Photos and work',
    hours: 'Opening hours',
  ...
  it: {
    ...
    introEyebrow: 'PRESENTAZIONE',
    introTitle: 'Un professionista vicino ai tuoi progetti',
    introText: 'Una presentazione chiara dell’attività, delle competenze e dei punti di forza.',
    read: 'Leggi la presentazione',
    services: 'Servizi proposti',
    photos: 'Foto e realizzazioni',
    hours: 'Orari',
  ...
  ru: {
    ...
    introTitle: 'Профессионал рядом с вашими проектами',
    introText: 'Понятное представление деятельности, опыта и сильных сторон специалиста.',
    read: 'Читать описание',
    services: 'Услуги',
    photos: 'Фотографии и работы',
    hours: 'Часы работы',
  ...
  const sections: PreviewSection[] = [
    {
      ...
      content: (
        <div className="dt-service-grid">
          <div className="dt-service-item">Buffets & événements</div>
          <div className="dt-service-item">Cuisine sur mesure</div>
          <div className="dt-service-item">Réceptions privées</div>
        </div>
      ),
    },
    {
      ...
      title: t.photos,
      icon: Images,
      badge: String(galleryCount),
      content: <div className="text-xs text-[#d8eee6]">{gallery || `${galleryCount} réalisations présentées`}</div>,
    },
    {
      id: 'hours',
      title: t.hours,
      icon: Clock3,
      badge: status,
      content: <div className="whitespace-pre-line text-xs leading-6 text-[#d8eee6]">{hours || 'Lun–Sam : 10:00–18:00'}</div>,
    },
    {
      id: 'practical',
      title: t.practical,
      icon: MapPin,
      content: (
        <div className="space-y-2 text-xs text-[#d8eee6]">
    ...
    },
    {
      ...
      icon: Star,
      badge: ratingLabel,
      content: <div className="text-xs text-[#d8eee6]">Google · 5.0/5 · 27 avis</div>,
    },
    {
      id: 'sharing',
      title: t.sharing,
      ...
      content: (
        <div className="flex items-center gap-3">
          <div className="grid h-16 w-16 shrink-0 place-items-center rounded-lg border border-[#D4AF37]/40 bg-white text-[#0f2d23]">
            <QrCode className="h-10 w-10" />
          </div>
          <p className="text-xs leading-5 text-[#d8eee6]">{t.qrText}</p>
        </div>
      ),
    },
  ];
    ...
    <div className={size === 'compact' ? 'mx-auto w-full max-w-[360px]' : 'mx-auto w-full max-w-[400px]'} dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <div className="dt-showcase" style={PREMIUM_THEME}>
        ...
        <section className="dt-actions">
          <div className="dt-primary-actions" style={{ gridTemplateColumns: 'repeat(4,minmax(0,1fr))' }}>
          ...
          <div className="dt-primary-actions mt-1.5" style={{ gridTemplateColumns: 'minmax(0,1fr)' }}>
            <button className="dt-primary-action" type="button" style={{ minHeight: 45 }}><Globe2 />{t.website}</button>
          </div>
              ...
              return (
                <section className={`dt-accordion ${open ? 'open' : ''}`} key={section.id}>
                  <button type="button" className="dt-accordion-trigger" onClick={() => toggle(section.id)} aria-expanded={open}>
                    ...
                    {section.badge && (
                      <small className={`dt-section-badge ${section.id === 'reviews' ? 'dt-google-review-badge' : ''}`}>
                        {section.badge}
                      </small>
                    )}
