...
const COPY: Record<BusinessCardPreviewLanguage, PreviewCopy> = {
  fr: {
    ...
    call: 'Appeler', whatsapp: 'WhatsApp', email: 'E-mail', directions: 'Itinéraire', website: 'Site web', book: 'Réserver', addContact: 'Ajouter aux contacts', contactNotice: 'Coordonnées fournies par le professionnel.',
  ...
  ar: {
    ...
    call: 'اتصال', whatsapp: 'واتساب', email: 'البريد الإلكتروني', directions: 'الاتجاهات', website: 'الموقع الإلكتروني', book: 'حجز', addContact: 'إضافة إلى جهات الاتصال', contactNotice: 'بيانات الاتصال مقدمة من المهني.',
  ...
  en: {
    ...
    call: 'Call', whatsapp: 'WhatsApp', email: 'Email', directions: 'Directions', website: 'Website', book: 'Book', addContact: 'Add to contacts', contactNotice: 'Contact details provided by the professional.',
  ...
  it: {
    ...
    call: 'Chiama', whatsapp: 'WhatsApp', email: 'Email', directions: 'Indicazioni', website: 'Sito web', book: 'Prenota', addContact: 'Aggiungi ai contatti', contactNotice: 'Contatti forniti dal professionista.',
  ...
  ru: {
    ...
    call: 'Позвонить', whatsapp: 'WhatsApp', email: 'Email', directions: 'Маршрут', website: 'Сайт', book: 'Забронировать', addContact: 'Добавить в контакты', contactNotice: 'Контакты предоставлены профессионалом.',
...
export default function BusinessCardPreview({
  ...
  logo = DEFAULT_LOGO, coverImage = DEFAULT_COVER, gallery, reviews, hours,
  ...
  const sections: PreviewSection[] = [
    ...
    { id: 'gallery', title: t.photos, icon: Images, badge: String(galleryCount), content: <div className="text-xs text-[#d8eee6]">{gallery || `${galleryCount} réalisations présentées`}</div> },
    ...
    <div className={size === 'compact' ? 'mx-auto w-full max-w-[360px]' : 'mx-auto w-full max-w-[400px]'} dir={language === 'ar' ? 'rtl' : 'ltr'}>
      <div className="dt-showcase" style={PREMIUM_THEME}>
        ...
        <section className="dt-actions">
          ...
          {actionsOpen && (
            <div className="dt-more-actions-panel">
              ...
              <button className="dt-primary-action" type="button"><Globe2 />{t.website}</button>
            </div>
          )}
