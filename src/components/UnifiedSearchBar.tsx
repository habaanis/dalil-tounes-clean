  ...
  const fetchSuggestions = async (query: string, ville: string) => {
    ...
    try {
      ...
      if (!rpcError && rpcData) {
        ...
        const mapped: EntrepriseRow[] = (rpcData as { id: string; nom: string; ville: string; categorie: string }[])
          .filter(r => {
            const nomNorm = (r.nom || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
            const catNorm = (r.categorie || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
            return nomNorm.includes(qNorm) || catNorm.includes(qNorm);
          })
          .slice(0, 10)
          .map(r => ({
            id: r.id,
