...
export default function BecomePartner() {
  ...
  return <div className="bg-white text-[#374151]" dir={rtl ? 'rtl' : 'ltr'}>
    <section className="relative overflow-hidden bg-gradient-to-br from-[#FFF8E6] via-white to-[#F4F8F6] px-4 py-16 md:py-24">
      <div className="mx-auto grid max-w-6xl items-center gap-12 lg:grid-cols-2">
        <div className="relative z-10">
          ...
          <a href="/contact?objet=partenariat" className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#005B44] px-6 py-3 font-bold text-white shadow-lg transition hover:-translate-y-0.5">{t.cta}<ArrowRight size={18}/></a>
        ...
        <div className="mx-auto grid w-full max-w-[440px] grid-cols-2 items-center gap-3" aria-label={t.solutionTitle}>
          <figure className="relative h-[390px] w-full -rotate-2 overflow-hidden rounded-[28px] border-[5px] border-[#07392E] bg-white shadow-2xl">
            ...
            <button type="button" onClick={() => setModelZoom({src:'/images/cv-business-portfolio-aux-saveurs-anis.png',label:t.professional})} className="absolute bottom-3 end-3 z-10 flex min-h-11 items-center gap-2 rounded-full border-2 border-[#F4CE55] bg-[#07392E] px-3 text-xs font-bold text-white shadow-xl" aria-label={`${t.professional} — ${rtl ? 'تكبير' : language === 'en' ? 'Enlarge' : language === 'it' ? 'Ingrandisci' : language === 'ru' ? 'Увеличить' : 'Agrandir'}`}><ZoomIn size={18}/><span className="hidden sm:inline">{rtl ? 'تكبير' : language === 'en' ? 'Enlarge' : language === 'it' ? 'Ingrandisci' : language === 'ru' ? 'Увеличить' : 'Agrandir'}</span></button>
          </figure>
          <figure className="relative h-[390px] w-full rotate-2 overflow-hidden rounded-[28px] border-[5px] border-[#4A1D43] bg-white shadow-2xl">
            ...
            <button type="button" onClick={() => setModelZoom({src:'/images/cv-business-professionnel-aux-saveurs-anis.png',label:t.portfolio})} className="absolute bottom-3 end-3 z-10 flex min-h-11 items-center gap-2 rounded-full border-2 border-[#F4CE55] bg-[#4A1D43] px-3 text-xs font-bold text-white shadow-xl" aria-label={`${t.portfolio} — ${rtl ? 'تكبير' : language === 'en' ? 'Enlarge' : language === 'it' ? 'Ingrandisci' : language === 'ru' ? 'Увеличить' : 'Agrandir'}`}><ZoomIn size={18}/><span className="hidden sm:inline">{rtl ? 'تكبير' : language === 'en' ? 'Enlarge' : language === 'it' ? 'Ingrandisci' : language === 'ru' ? 'Увеличить' : 'Agrandir'}</span></button>
          </figure>
        </div>
      </div>
    </section>
    ...
    <Section eyebrow={t.offersEyebrow} title={t.offersTitle} intro={t.offersIntro}>
      <div className="grid gap-5 lg:grid-cols-3">
        <Offer rate={0} title={t.contact} text={t.contactText} tasks={t.contactTasks} neutral/>
      ...
      <div className="mt-5 flex flex-col gap-2 rounded-2xl border border-[#D4AF37]/40 bg-[#FFF8E6] p-5 sm:flex-row"><strong className="shrink-0 text-[#4A1D43]">{t.commonRule}</strong><span className="text-gray-700">{t.commonRuleText}</span></div>
      <p className="mt-4 text-xs leading-6 text-gray-500">{t.legal}</p>
    </Section>
    <div className="bg-[#F7F3EA]"><Section eyebrow={t.salePathEyebrow} title={t.salePathTitle}>
      <ol className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{t.salePath.map(([title,text], index) => <li key={title} className="rounded-2xl border border-[#D4AF37]/30 bg-white p-5"><b className="grid h-9 w-9 place-items-center rounded-full bg-[#F4CE55] text-[#4A1D43]">{index+1}</b><h3 className="mt-4 text-lg font-bold text-[#4A1D43]">{title}</h3><p className="mt-2 text-sm leading-6 text-gray-600">{text}</p></li>)}</ol>
      <div className="mt-6 flex flex-col gap-2 rounded-2xl bg-[#07392E] p-5 text-white sm:flex-row sm:flex-wrap sm:items-center"><strong className="me-2">{t.trackingTitle}</strong>{t.tracking.map(status=><span key={status} className="rounded-full bg-white/10 px-3 py-2 text-sm">{status}</span>)}</div>
    </Section></div>
    ...
    <section id="partenaire-formulaire" className="scroll-mt-24 bg-[#F7F3EA] px-4 py-16 md:py-24">
      <div className="mx-auto max-w-4xl rounded-3xl border border-[#D4AF37]/40 bg-white p-6 text-center shadow-sm md:p-10">
        <Eyebrow>{t.formEyebrow}</Eyebrow>
        <h2 className="mt-3 text-3xl font-bold text-[#4A1D43] md:text-5xl">{t.formTitle}</h2>
        <p className="mx-auto mt-5 max-w-2xl leading-8 text-gray-600">{t.formIntro}</p>
        <a href="/contact?objet=partenariat" className="mt-7 inline-flex items-center gap-2 rounded-full bg-[#005B44] px-6 py-3 font-bold text-white shadow-lg transition hover:-translate-y-0.5">{t.cta}<ArrowRight size={18}/></a>
      </div>
    </section>
    ...
    <section className="bg-[#4A1D43] px-4 py-16 text-center text-white"><h2 className="text-3xl font-bold md:text-5xl">{t.finalTitle}</h2><p className="mx-auto mt-4 max-w-2xl leading-8 text-white/80">{t.finalText}</p><a href="/contact?objet=partenariat" className="mt-6 inline-flex items-center gap-2 rounded-full bg-[#D4AF37] px-6 py-3 font-bold text-[#241020]">{t.cta}<ArrowRight size={18}/></a></section>
  </div>;
}
...
function Offer({rate,title,text,tasks,featured,neutral}:{rate:number;title:string;text:string;tasks:readonly string[];featured?:boolean;neutral?:boolean}) { return <article className={`relative rounded-3xl border p-7 text-start ${featured?'border-[#D4AF37] bg-[#07392E] text-white':neutral?'border-[#D4AF37]/30 bg-[#F7F3EA]':'border-gray-200 bg-white'}`}><span className="absolute end-5 top-5 rounded-full bg-[#F4CE55] px-3 py-1 font-black text-[#4A1D43]">{rate}%</span><h3 className={`pe-16 text-2xl font-bold ${featured?'text-white':'text-[#4A1D43]'}`}>{title}</h3><p className={`mt-3 leading-7 ${featured?'text-white/80':'text-gray-600'}`}>{text}</p><ul className="mt-5 space-y-3">{tasks.map(task=><li key={task} className="flex items-start gap-2"><Check className="mt-1 shrink-0 text-[#D4AF37]" size={17}/>{task}</li>)}</ul></article>; }
