  ...
  return (
    <article className={`mx-auto w-full overflow-hidden text-white ${isPremium ? 'max-w-[400px] rounded-[24px] border-2 border-[#E0B93E] bg-[radial-gradient(circle_at_50%_8%,rgba(20,111,77,0.34),transparent_28%),linear-gradient(145deg,#031D18_0%,#042D24_50%,#011914_100%)] p-1.5 shadow-[0_20px_44px_rgba(0,24,19,0.48),0_0_12px_rgba(224,185,62,0.16),inset_0_0_24px_rgba(224,185,62,0.07)]' : 'max-w-[410px] rounded-[25px] border border-[#CDA82F] bg-[radial-gradient(circle_at_35%_0%,rgba(17,103,74,0.28),transparent_34%),linear-gradient(180deg,#064735_0%,#032F27_48%,#021F1A_100%)] shadow-[0_16px_34px_rgba(0,38,29,0.38),0_0_7px_rgba(205,168,47,0.1),inset_0_0_18px_rgba(0,0,0,0.2)]'}`}>
      <div className={`relative overflow-hidden ${isPremium ? 'h-[88px] rounded-t-[19px] border border-[#D6AF2E]/40' : 'h-20'}`}>
        ...
        <div className={`absolute inset-0 ${isPremium ? 'bg-gradient-to-t from-[#021E19] via-black/10 to-black/10' : 'bg-gradient-to-t from-[#043A2D] via-transparent to-black/5'}`} />
      </div>
      ...
      <div className={`${isPremium ? '-mt-5 rounded-[20px] border border-[#D9B43A]/70 bg-[radial-gradient(circle_at_50%_8%,rgba(17,92,67,0.25),transparent_28%),linear-gradient(180deg,rgba(3,42,34,0.98),rgba(1,25,21,0.99))] px-3 pb-3 pt-0 shadow-[0_0_11px_rgba(224,185,62,0.12),inset_0_0_18px_rgba(0,0,0,0.3)]' : 'px-3.5 pb-3'} relative`}>
        ...
        {!isPremium && <>
          <section className="mt-1 rounded-xl border border-[#C6A333]/45 bg-[#011B17]/80 px-2.5 py-1.5 text-center shadow-[inset_0_0_10px_rgba(0,0,0,0.2)]" aria-label={labels.recommend}>
        ...
        {isPremium && <>
          ...
          <div className="mt-2 flex justify-center gap-6 border-t border-[#D6AF2E]/35 pt-2">{[Phone, MessageCircle, Navigation].map((Icon, index) => <button key={index} type="button" onClick={() => activate(labels.actions[index])} aria-label={labels.actions[index]} className="flex h-9 w-9 items-center justify-center rounded-full border border-[#E0B93E]/80 bg-[radial-gradient(circle_at_40%_30%,#084B39,#011C17_70%)] text-[#F4CE55] shadow-[0_0_8px_rgba(224,185,62,0.25),inset_0_1px_5px_rgba(244,206,85,0.1)] transition hover:scale-105 hover:shadow-[0_0_11px_rgba(224,185,62,0.34)] active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-white"><Icon className="h-4 w-4" aria-hidden="true" /></button>)}</div>
        </>}
