...
page = Path('src/pages/BusinessQr.tsx')
...
router = Path('src/AppRouter.tsx')
s = router.read_text()
marker = "const BusinessDetail = lazy(() => import('./components/BusinessShowcaseDetail'));\n"
if marker not in s:
    raise SystemExit('BusinessDetail lazy marker not found')
s = s.replace(marker, marker + "const BusinessQr = lazy(() => import('./pages/BusinessQr'));\n", 1)
route_marker = '          <Route path="/p/:slug" element={<BusinessDetail />} />\n'
if route_marker not in s:
    raise SystemExit('Business routes marker not found')
s = s.replace(route_marker, route_marker + '          <Route path="/qr-business/:id" element={<BusinessQr />} />\n', 1)
router.write_text(s)

detail = Path('src/components/BusinessShowcaseLienoraDetail.tsx')
s = detail.read_text()
old = '''      <div className="dt-qr-actions">\n        <button type="button" className="dt-qr-action" onClick={() => void share()}>\n'''
new = '''      <div className="dt-qr-actions">\n        {capabilities.variant === 'premium' && (\n          <Link className="dt-qr-action" to={`/qr-business/${business.id}`}>\n            <QrCode aria-hidden="true" />\n            {language === 'ar' ? 'عرض رمز QR الخاص بي' : language === 'en' ? 'Present my Business QR' : language === 'it' ? 'Mostra il mio QR Business' : language === 'ru' ? 'Показать мой Business QR' : 'Présenter mon QR Business'}\n          </Link>\n        )}\n        <button type="button" className="dt-qr-action" onClick={() => void share()}>\n'''
if old not in s:
    raise SystemExit('QR actions block not found')
s = s.replace(old, new, 1)
detail.write_text(s)

subscription = Path('src/pages/Subscription.tsx')
s = subscription.read_text()
marker = 'const offerCopy:'
pos = s.find(marker)
if pos < 0:
    raise SystemExit('offerCopy not found')
...
replacements = [
    ...
    ("'Dalil Tounes Certified badge', 'Up to 10 photos'", "'Dalil Tounes Certified badge', 'Business QR to present from your phone — your customer scans it and opens your Business CV directly', 'Up to 10 photos'"),
    ("'شارة دليل تونس موثّق', 'حتى 10 صور'", "'شارة دليل تونس موثّق', 'رمز QR للأعمال لعرضه من هاتفك — يمسحه العميل ويفتح مباشرة السيرة المهنية لنشاطك', 'حتى 10 صور'"),
    ("'Badge Dalil Tounes Certificato', 'Fino a 10 foto'", "'Badge Dalil Tounes Certificato', 'QR Business da mostrare dal telefono — il cliente lo scansiona e apre direttamente il tuo CV Business', 'Fino a 10 foto'"),
    ("'Значок «Проверено Dalil Tounes»', 'До 10 фотографий'", "'Значок «Проверено Dalil Tounes»', 'Business QR для показа с телефона — клиент сканирует его и сразу открывает ваш Business CV', 'До 10 фотографий'"),
]
for old_text, new_text in replacements:
    if old_text not in tail:
        raise SystemExit(f'Offer copy target not found: {old_text[:45]}')
    tail = tail.replace(old_text, new_text, 1)
subscription.write_text(head + tail)
