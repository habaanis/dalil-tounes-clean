...
const ANNUAL_PAYMENT_SCHEDULES = new Set(['annual_single', 'annual_three']);
const CONTACT_METHODS = new Set(['whatsapp', 'phone', 'email']);
const CONTACT_TIMES = new Set(['morning', 'afternoon', 'evening', 'any']);
...
export function validateRegistrationRequest(raw: RawRegistrationRequest): ValidationResult {
  ...
  if (mode === 'subscription') {
    ...
    if (!selectedPlan) return invalid('Formule sélectionnée obligatoire.', 'selectedPlan');
